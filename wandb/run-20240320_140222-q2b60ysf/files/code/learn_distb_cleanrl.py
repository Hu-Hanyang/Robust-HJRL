...
class FlattenAction(gym.ActionWrapper, gym.utils.RecordConstructorArgs):
...
class Args:
    ...
    wandb_project_name: str = "PPO-HJ-distbs-ClearnRL"
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_mean = nn.Sequential(
            layer_init(nn.Linear(np.array(envs.single_observation_space.shape).prod(), 50)),
            nn.ReLU(),
            layer_init(nn.Linear(50, 50)),
            nn.ReLU(),
            layer_init(nn.Linear(50, np.prod(envs.single_action_space.shape)), std=0.01),
        )
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(f"{run_name}/tb")
    writer.add_text(
        "hyperparameters",
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.disturbance_type, args.disturbance_level, i, args.capture_video, run_name) for i in range(args.num_envs)]
    )
