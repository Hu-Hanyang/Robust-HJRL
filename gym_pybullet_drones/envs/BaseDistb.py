...
from gym_pybullet_drones.utils.utils import Boltzmann
from gym_pybullet_drones.hj_distbs.FasTrack_data.distur_gener import distur_gener, quat2euler


class BaseDistbEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 initial_xyzs=None,
                 initial_rpys=None,
                 physics: Physics=Physics.PYB,  #  Base PyBullet physics update
        ...
        randomization_reset : bool, optional
            Whether to randomize the initial conditions at each reset.
        ...
        if self.PYB_FREQ % self.CTRL_FREQ != 0:
            raise ValueError('[ERROR] in BaseDistbEnv.__init__(), pyb_freq is not divisible by env_freq.')
        self.PYB_STEPS_PER_CTRL = int(self.PYB_FREQ / self.CTRL_FREQ)  # The number of the control loop iterations per PyBullet step
        ...
        self.distb_type = disturbance_type
        if self.distb_type not in [None, 'fixed', 'boltzmann', 'random', 'rarl', 'rarl-population']:
            raise ValueError('[ERROR] in BaseDistbEnv.__init__(), invalid disturbance type.')
        self.distb_level = distb_level
        self.randomization_reset = randomization_reset
        ...
        print("[INFO] BaseDistbEnv.__init__() loaded parameters from the drone's .urdf:\n[INFO] m {:f}, L {:f},\n[INFO] ixx {:f}, iyy {:f}, izz {:f},\n[INFO] kf {:f}, km {:f},\n[INFO] t2w {:f}, max_speed_kmh {:f},\n[INFO] gnd_eff_coeff {:f}, prop_radius {:f},\n[INFO] drag_xy_coeff {:f}, drag_z_coeff {:f},\n[INFO] dw_coeff_1 {:f}, dw_coeff_2 {:f}, dw_coeff_3 {:f}".format(
        ...
        if initial_xyzs is None:
            ...
            print("[ERROR] invalid initial_xyzs in BaseDistbEnv.__init__(), try initial_xyzs.reshape(NUM_DRONES,3)")
        if initial_rpys is None:
            self.INIT_RPYS = np.zeros((self.NUM_DRONES, 3))
        elif np.array(initial_rpys).shape == (self.NUM_DRONES, 3):
            self.INIT_RPYS = initial_rpys
        else:
            print("[ERROR] invalid initial_rpys in BaseDistbEnv.__init__(), try initial_rpys.reshape(NUM_DRONES,3)")
    ...
    def render(self,
        ...
        if self.first_render_call and not self.GUI:
            print("[WARNING] BaseDistbEnv.render() is implemented as text-only, re-initialize the environment using Aviary(gui=True) to use PyBullet's graphical interface")
            self.first_render_call = False
        print("\n[INFO] BaseDistbEnv.render() ——— it {:04d}".format(self.step_counter),
        ...
        for i in range (self.NUM_DRONES):
            print("[INFO] BaseDistbEnv.render() ——— drone {:d}".format(i),
    ...
    def _housekeeping(self):
        ...
        # print(f"The initial self.last_clipped_action is {self.last_clipped_action}")
        # print(f"The initial self.last_clipped_action shape is {self.last_clipped_action.shape}")
        ...
        #### Connect to PyBullet ###################################
            ...
            for id in range(self.NUM_DRONES):
                ...
                p.resetBaseVelocity(self.DRONE_IDS[id], linearVelocity=self.vel[id], angularVelocity=R.T@self.ang_v[id], physicsClientId=self.CLIENT)
        
        # Hanyang: check the initialization randomization
        # state = self._getDroneStateVector(0)
        # print(f"The initial state (20, ) is {state}")
    ...
    def _getDroneImages(self,
        ...
        if self.IMG_RES is None:
            print("[ERROR] in BaseDistbEnv._getDroneImages(), remember to set self.IMG_RES to np.array([width, height])")
            exit()
    ...
    def _exportImage(self,
        ...
        if img_type == ImageType.RGB:
            ...
            print("[ERROR] in BaseDistbEnv._exportImage(), unknown ImageType")
            exit()
        if img_type != ImageType.RGB:
    ...
    def _normalizedActionToRPM(self,
        ...
        if np.any(np.abs(action) > 1):
            print("\n[ERROR] it", self.step_counter, "in BaseDistbEnv._normalizedActionToRPM(), out-of-bound action")
